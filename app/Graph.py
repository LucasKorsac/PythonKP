...
import numpy as np
from app.TimingAttack import TimingAttack
...
def PlotRes(df):  # График зависимости среднего времени факторизации от размера ключа.
    ...
    for (c, strong), group in df.groupby(["c", "strong"]):
        grouped = group.groupby("bit")[
            "time"
        ].mean()  # Усреднение времени по размеру ключа

        label = f"простые={c}, сильные={strong}"  # Подписи

        # Построение линии
        plt.plot(
            grouped.index, grouped.values, marker="o", linewidth=2, label=label
        )  # Передача значний и обозначения в графике
...
def PlotWithTheory(
    df,
):  # Сравнение экспериментальных данных с теоретическим ростом сложности.
    plt.figure(figsize=(8, 5))
    ...
    plt.plot(
        grouped.index, grouped.values, marker="o", linewidth=2, label="Эксперимент"
    )
    ...
    theory = (
        theory / max(theory) * max(grouped.values)
    )  # Нормировка теоретической кривой
...
def PlotExperiments(df):  # Отображение экспериментов
    ...
    for (c, strong), group in df.groupby(["c", "strong"]):
        ...
        plt.scatter(
            group["bit"], group["time"], s=100, marker=markers[c], label=label
        )  # Отображение точек экспериментов
...
# График для атаки по времени
def PlotTimingAttack(res):

    base = 5
    mod = res["n"]  # Модуль из интерфейса
    secret = res["d"]  # Секретный ключ RSA

    bits = []
    times = []

    keyb = secret.bit_length()  # длина ключа

    for i in range(1, keyb + 1):
        # первые i бит секретного ключа
        exp = secret >> (keyb - i)

        t = TimingAttack.MeasureTime(base, exp, mod)
    ...
    plt.xlabel("Количество бит секретного ключа")
    plt.ylabel("Время выполнения (сек)")
    plt.title("Атака по времени RSA")
