...
class Gui:
    ...
    def __init__(self, root):
        ...
        # кнопки выбора
        ttk.Radiobutton(
            radfr, text="Обычные", variable=self.prtype, value="normal"
        ).pack(side="left", padx=10)
        ttk.Radiobutton(
            radfr, text="Близкие", variable=self.prtype, value="close"
        ).pack(side="left", padx=10)
        ttk.Radiobutton(
            radfr, text="Сильные", variable=self.prtype, value="strong"
        ).pack(side="left", padx=10)

        ttk.Button(root, text="Запустить эксперимент", command=self.RunExper).pack(
            pady=10
        )  # Запуск эксперимента
        ...
        # Кнопка генерации
        ttk.Button(btnfr, text="Сгенерировать", command=self.GenWindow).pack(
            side="left", padx=10
        )

        # Кнопка графиков
        ttk.Button(btnfr, text="Графики", command=self.GraphWindow).pack(
            side="left", padx=10
        )
    ...
    def ExperThread(self):
        try:
            ...
            df = Experiment.run(
                samp=samp, bit=bit, c=close, st=strong
            )  # Запуск эксперимента
    ...
    def GenWindow(self):
        ...
        ttk.Radiobutton(
            genwin,
            text="Простое число по Миллеру-Рабину",
            variable=typeprine,
            value="normal",
        ).pack(pady=5)
        ttk.Radiobutton(
            genwin, text="Сильное простое", variable=typeprine, value="strong"
        ).pack(pady=5)
        ...
        ttk.Button(btnfr, text="ОК", command=Generate).pack(
            side="left", padx=10
        )  # Запуск генерации
        ttk.Button(btnfr, text="Отмена", command=genwin.destroy).pack(
            side="left", padx=10
        )  # Закрытие окна
    ...
    def GraphWindow(self, rsa=None):
        ...
        ttk.Button(
            graphwin, text="Среднее время", command=lambda: Graph.PlotRes(self.res)
        ).pack(pady=5)

        # Вызов графика по всем экспериментам
        ttk.Button(
            graphwin,
            text="Все эксперименты",
            command=lambda: Graph.PlotExperiments(self.res),
        ).pack(pady=5)
        ...
        ttk.Button(
            ...
            command=lambda: Graph.PlotTimingAttack(
                {"n": self.rsa.n, "e": self.rsa.e, "d": self.rsa.d}
            ),
        ).pack(pady=5)
